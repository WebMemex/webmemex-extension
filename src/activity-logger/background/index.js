...
let pageLoaded = function(item){
    pages.forEach((page)=>{
        if(item.url == page.url && item.tabId == page.tabId){
...
let navigated = function(details){
    ...
    logPageVisit({
        ...
        pages.push({tabId: details.tabId, url: details.url});
...
browser.webNavigation.onHistoryStateUpdated.addListener((details)=>{
    browser.tabs.get(details.tabId,(tab)=>{
        ...
        //make sure the page logged is the the right page by checking the url
